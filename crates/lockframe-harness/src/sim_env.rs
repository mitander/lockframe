...
///
/// `now()` returns Turmoil's simulated time, which can be advanced instantly
/// via `turmoil::sleep()`. `random_bytes()` uses `ChaCha20Rng` seeded with a
/// fixed value (0 by default), ensuring reproducible test runs and easier
/// debugging.
#[derive(Clone)]
pub struct SimEnv {
